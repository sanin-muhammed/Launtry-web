...
const PickupPoint = ({ addresses, fetchAddress }) => {
  ...
  const setId = (id) => {
    ...
    if (findedItem) {
      ...
      const { userId, destination, address, phoneNumber } = findedItem;
      setFormData({
        userId,
        destination,
        address,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.userId) {
      enqueueSnackbar("undefined userId", { variant: "error" });
      return;
        ...
        <div className="offers">
          {addresses.length > 0 ? (
            addresses.map((item, index) => (
              <div
                className="offer select_address "
                id={address === item._id ? "set" : ""}
                key={index}
